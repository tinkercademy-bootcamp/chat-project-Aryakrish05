...
void check_and_handle_error(bool test, std::string error_message,int sock) {
  if (test) {
...
int create_socket() {
  int my_sock;
  int error_code= my_sock = socket(AF_INET, SOCK_STREAM, 0);
  check_and_handle_error(error_code<0,"Socket creation error\n",my_sock);
  return my_sock;
}

bool set_socket_options(int sock, int opt) {
  int error_code=setsockopt(sock, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt,
                 sizeof(opt));
  check_and_handle_error(error_code<0,"setsockopt() error\n",sock);
  return true;
}
...
void bind_address_to_socket(int sock, sockaddr_in &address) {
  int error_code=bind(sock, (sockaddr *)&address, sizeof(address));
  check_and_handle_error(error_code<0,"bind failed\n",sock);
}

void listen_on_socket(int sock) {
  int error_code=listen(sock, 3);
  check_and_handle_error(error_code<0, "listen failed\n", sock);
}
